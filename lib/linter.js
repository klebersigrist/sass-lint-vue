...
class Linter {
  ...
  extractFileTemplates (fileData) {
    ...
    const handler = new htmlparser.DefaultHandler((error, dom) => {
      ...
      const scssTemplate = $('style[lang="scss"]').toArray().reduce((result, element) => {
        return result + element.children[0].data
      }, '')

      const sassTemplate = $('style[lang="sass"]').toArray().reduce((result, element) => {
        return result + element.children[0].data
      }, '')
      ...
      if (scssTemplate && scssTemplate.length) {
        templates.push({
          content: baseIndent ? scssTemplate.replace(regex, '') : scssTemplate,
          format: 'scss',
      ...
      if (sassTemplate && sassTemplate.length) {
        templates.push({
          content: baseIndent ? sassTemplate.replace(regex, '') : sassTemplate,
          format: 'sass',
